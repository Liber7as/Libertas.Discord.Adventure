...
public class MobPresetServiceAdapter(IDbContextFactory<AdventureContext> dbContextFactory, IRandomNumberGenerator rng, ILogger<MobPresetServiceAdapter> logger) : IMobPresetService
{
    ...
    /// </summary>
    public async Task<MobPreset?> GetByIdAsync(int id, CancellationToken cancellationToken = default)
    {
        await using var ctx = await _dbContextFactory.CreateDbContextAsync(cancellationToken);
        return await ctx.Set<MobPreset>().FindAsync([id], cancellationToken);
    }

    /// <summary>
    ///     Returns all mob preset entities.
    /// </summary>
    public async Task<IReadOnlyList<MobPreset>> GetAllAsync(CancellationToken cancellationToken = default)
    {
        await using var ctx = await _dbContextFactory.CreateDbContextAsync(cancellationToken);
        return await ctx.Set<MobPreset>().AsNoTracking().ToListAsync(cancellationToken);
    }
}