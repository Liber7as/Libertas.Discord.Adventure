...
///     <para>
///         Access to session state should be synchronized using <see cref="SemaphoreSlim" />
///         or the helper extension methods defined in <see cref="AdventureSessionExtensions" />.
///     </para>
/// </remarks>
public record AdventureSession
{
    ...
    /// </summary>
    public List<MobState> Mobs { get; set; } = new();

    /// <summary>
    ///     Actions submitted by players for the upcoming round.
    ///     Keyed by player ID. Cleared after each round resolves.
    /// </summary>
    public Dictionary<PlayerId, PlayerAction> PendingActions { get; set; } = new();
